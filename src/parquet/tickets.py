...
def recent_proposals(storage: Storage, *, limit: int = 20) -> list[ProposalRecord]:
    rows = storage.conn.execute(
        "SELECT analysis_id, payload FROM proposals ORDER BY rowid DESC LIMIT ?", (limit,)
    ).fetchall()
    return [ProposalRecord(analysis_id=str(row[0]), proposal=TradeProposal.model_validate_json(str(row[1]))) for row in rows]


def choose_real_small_amount(*, gate_maximum_usd: float, supervised_cap_usd: float,
                             broker_minimum_usd: float | None,
                             requested_amount_usd: float | None = None) -> tuple[float, float]:
    ...
    if broker_minimum_usd is not None:
        ...
        if broker_minimum_usd > maximum_safe:
            raise RuntimeError(f"eToro minimum {broker_minimum_usd:.2f} USD exceeds the safe supervised maximum {maximum_safe:.2f} USD for this proposal")
    if requested_amount_usd is None:
        chosen_amount = broker_minimum_usd if broker_minimum_usd is not None else min(10.0, maximum_safe)
    else:
    ...
    if broker_minimum_usd is not None and chosen_amount < broker_minimum_usd:
        raise RuntimeError(f"Requested amount {chosen_amount:.2f} USD is below eToro minimum {broker_minimum_usd:.2f} USD")
    if chosen_amount > maximum_safe:
        raise RuntimeError(f"Requested amount {chosen_amount:.2f} USD exceeds safe maximum {maximum_safe:.2f} USD")
    return chosen_amount, maximum_safe


def validate_what_if_costs(costs: EtoroCostResult, *, instrument_id: int, amount_usd: float,
                           now: datetime, max_age: timedelta = timedelta(minutes=2)) -> None:
    ...
    if age < timedelta(seconds=-5) or age > max_age:
        raise RuntimeError(f"eToro what-if costs are stale or future-dated: age={age.total_seconds():.1f}s")
    total = costs.total_usd
    if total < 0:
        raise RuntimeError("eToro what-if cost total is negative")
    if total >= amount_usd:
        raise RuntimeError(f"eToro what-if costs {total:.2f} USD consume the full {amount_usd:.2f} USD ticket")


async def prepare_real_small_ticket(settings: Settings, *, proposal_id: str,
                                    requested_amount_usd: float | None = None) -> PreparedRealSmallTicket:
    ...
    reconciliation = ReconciliationService(settings, orchestrator.storage, orchestrator.market_client)
    ...
    decision = orchestrator.execution_gate.evaluate(proposal, snapshot, observation, now=now)
    ...
    chosen_amount, maximum_safe = choose_real_small_amount(
        ...
        broker_minimum_usd=broker_minimum, requested_amount_usd=requested_amount_usd)
    costs = await execution_client.what_if_open_costs(
        transaction="buy" if proposal.side == Side.BUY else "sellShort",
        instrument_id=instrument_id, settlement_type=settlement_type, amount_usd=chosen_amount,
        stop_loss_rate=proposal.stop_loss,
        take_profit_rate=proposal.take_profit,
        leverage=1,
    )
    validate_what_if_costs(costs, instrument_id=instrument_id, amount_usd=chosen_amount,
                           now=datetime.now(UTC))
    capped_decision = replace(decision, amount_usd=chosen_amount)
    attempt = orchestrator.autonomous_execution.prepare(
        proposal=proposal, watch_id=f"manual-real-small:{proposal.proposal_id}",
        observation=observation, decision=capped_decision, now=now)
    return PreparedRealSmallTicket(attempt=attempt, proposal=proposal, observation=observation,
        gate_decision=decision, eligibility=eligibility, costs=costs,
        broker_minimum_usd=broker_minimum, maximum_safe_amount_usd=maximum_safe)


async def _resolve_instrument_id(settings: Settings, market: EtoroMarketDataClient, symbol: str) -> int:
    normalized = symbol.upper()
    configured = settings.etoro.instrument_ids.get(normalized)
    if configured is not None:
        return configured
    hits = await market.search(normalized)
    exact_ids = sorted({hit.instrument_id for hit in hits if hit.symbol is not None and hit.symbol.upper() == normalized})
    if len(exact_ids) != 1:
        raise RuntimeError(f"Expected one exact eToro instrument for {normalized}, found {len(exact_ids)}")
    return exact_ids[0]
...
def _observation(proposal: TradeProposal, rate: InstrumentRate, instrument_id: int) -> MarketObservation:
    if rate.bid is None or rate.ask is None:
        raise RuntimeError("Bid/ask unavailable")
    price = rate.last_price
    if price is None:
        price = (rate.bid + rate.ask) / 2
    return MarketObservation(symbol=proposal.symbol, price=price,
        observed_at=rate.timestamp.astimezone(UTC), instrument_id=instrument_id, bid=rate.bid, ask=rate.ask)


def _execution_client(settings: Settings) -> EtoroExecutionClient:
    return EtoroExecutionClient(api_key=_read_secret(settings.etoro.api_key_file, "eToro API key"),
        user_key=_read_secret(settings.etoro.user_key_file, "eToro User key"),
        base_url=settings.etoro.execution_base_url, identity_base_url=settings.etoro.base_url)
