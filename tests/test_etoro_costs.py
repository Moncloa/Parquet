...
from parquet.execution.etoro import EtoroCostComponent, EtoroCostResult, EtoroExecutionClient
...
async def test_what_if_open_costs_uses_v2_cost_endpoint() -> None:
    ...
    client = EtoroExecutionClient(api_key="a", user_key="u", transport=httpx.MockTransport(handler))
    result = await client.what_if_open_costs(transaction="short", instrument_id=27,
        settlement_type="cfd", amount_usd=25, stop_loss_rate=7800, leverage=1)
...
def test_validate_what_if_costs_rejects_stale_response() -> None:
    ...
    costs = EtoroCostResult(request_id="r", instrument_id=27, symbol="SPX500",
        costs=(EtoroCostComponent("marketSpread", 0.1, "USD"),),
        last_updated=now - timedelta(minutes=3), response={})
...
def test_validate_what_if_costs_rejects_non_usd() -> None:
    now = datetime(2026, 9, 7, 20, 5, tzinfo=UTC)
    costs = EtoroCostResult(request_id="r", instrument_id=27, symbol="SPX500",
        costs=(EtoroCostComponent("tax", 0.1, "EUR"),), last_updated=now, response={})
